...
- Outgoing (nửa dưới, màu xanh dương) — traffic đang đi ra khỏi máy. Ví dụ đang upload, server trả response về cho client.
  ...
  - Đồ thị `#` càng cao càng rộng nghĩa là băng thông càng lớn tại thời điểm đó — nhìn hình dạng đồ thị là thấy ngay lúc nào mạng bận lúc nào rảnh.
...
`nload` mặc định hiển thị MBit/s (megabit), không phải MByte/s (megabyte). Hai cái này khác nhau 8 lần.
```bash
1 MByte/s = 8 MBit/s
```
- Đổi sang MB
