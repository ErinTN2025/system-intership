...
Đây là một tính năng của Dockerfile cho phép bạn sử dụng nhiều FROM instruction trong cùng một Dockerfile. Mỗi FROM bắt đàu một stage mới, và bạn có thể sao chép artifacts từ stage này sang stage khác.

**Advantages**

| Vấn đề                                        | Giải pháp                            |
| --------------------------------------------- | ------------------------------------ |
| Image quá lớn vì chứa cả build tools          | Chỉ giữ lại binary/runtime cần thiết |
| Lộ source code, dependencies trong image cuối | Build ở stage riêng, chỉ copy output |
| Security risk từ dev dependencies             | Production image chỉ chứa runtime    |

### 3.2 Lúc build 
- Chúng ta lấy lúc build của multi-stage, docker sẽ build theo từng layer, từng stage 1
```plain
Stage 1: frontend-build ──► Stage 2: backend-build ──► Stage 3: nginx:alpine (final)
     │                            │                          │
     ▼                            ▼                          ▼
  Tạo container              Tạo container              Tạo container
  tạm thời, chạy             tạm thời, chạy             tạm thời, chạy
  npm run build                npm run build              copy files
  → Tạo ra image               → Tạo ra image             → Tạo ra image
    tạm (intermediate)           tạm (intermediate)         cuối cùng
```
Các stage tạm được lưu ở
- Local Docker image cache (trên máy local)
- Mỗi stage tạo ra một intermediate image (không có tag, chỉ có ID)

Lúc build xong, mặc định chỉ có stage cuối cùng được giữ lại dưới dạng image.
- Nếu muốn giữ lại stage cụ thể, dùng `--target`:
```bash
# Chỉ build đến stage frontend-build và giữ lại nó
docker build --target frontend-build -t myapp-frontend:latest .

# Chỉ build đến stage backend-build và giữ lại nó
docker build --target backend-build -t myapp-backend:latest .
```

### 3.3 Lệnh `COPY . .`

| Vị trí            | Ý nghĩa                                                 | Gọi là gì                |
| ----------------- | ------------------------------------------------------- | ------------------------ |
| **Chấm 1** (src)  | Thư mục **build context** — nơi bạn chạy `docker build` | **Host** (máy của bạn)   |
| **Chấm 2** (dest) | Thư mục **trong container** đang build                  | **Container filesystem** |

Lưu ý lại 1 chút container đã tồn tại từ khi chúng ta build chỉ là dưới dạng Container tạm thời:
```bash
docker build -t myapp .    ← bạn chạy lệnh này
            │
            ▼
    Docker daemon tạo ra một container tạm thời
    từ image của stage hiện tại (ví dụ: node:18)
            │
            ├──► Container này có filesystem riêng
            │    (chưa có gì ngoài image gốc)
            │
            └──► COPY . .  ← chép từ host vào container này
```