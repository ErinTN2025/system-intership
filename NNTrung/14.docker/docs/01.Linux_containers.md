...
|-----------|-----------------|---------------|
| PID | Cô lập Process ID | Container 1 thấy PID 1 là init của nó |
| Mount (mnt) | Cô lập hệ thống file Filesystem mount| Container có root filesystem riêng|
| Network (net)| Cô lập network stack (Card mạng, IP, routing, port)| Mỗi container có IP riêng hoặc dùng bridge |
...
> NS: ID namespace, inode number của object namespace trong kernel, tra cứu: `readlink /proc/5360/ns/net`
> TYPE: Loại namespace
> NPROC: process đang nằm trong đó.
> PID: là một process đại diện mà lsns chọn để hiển thị, Kernel dùng process này để suy ra namespace.
> USER: user sở hữu process đại diện
> COMMAND: command của process đại diện
