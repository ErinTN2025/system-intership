...
Nếu permission denied nghĩa là docker của bạn ssh theo kiểu non-interactive -> dùng keypairs.
...
**Đổi tên**:
```bash
docker context rename my-remote-engine docker-server
```
**Điều tra**
```bash
...
- `~/.docker/contexts/meta` là thư mục chứa metadata của các context.
- Thư mục có tên dài là ID/hash, không phải tên context.
- Tên my-remote-engine được lưu bên trong file meta.json.
- Muốn quản lý context thì dùng các lệnh `docker context ls`, `docker context inspect`, `docker context  rm` thay vì thao tác trực tiếp trong thư mục này.
...
```
2 thành phần trên:
- SAN = Subject Alternative Name, Nó là trường mở rộng trong chứng chỉ X.509 dùng để khai báo: Chứng chỉ này hợp lệ cho những hostname/IP nào?

- "Chứng chỉ này được phép dùng cho server TLS" → EKU
...
- Sửa daemon.json: `sudo nano /etc/docker/daemon.json`
```JSON
{
    "hosts": [
...
Lưu ý: Trên Ubuntu/Debian, nếu Docker được khởi động qua `systemd` với `ExecStart` sẵn có tham số -H, daemon.json có thể bị xung đột (lỗi "unable to configure the Docker daemon with file"). Đây là lỗi hosts được khai báo ở 2 nơi.

Khi đó cần override service:
...
```
- Để quay lại: `unset DOCKER_HOST`, `unset DOCKER_TLS_VERIFY`
