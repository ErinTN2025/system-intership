...
### 1. Khái niệm
KVM( Kernel-based Virtual Machine) là một công nghệ ảo hóa mã nguồn mở cho hệ điều hành Linux, một hypervisor tích hợp trực tiếp vào kernel Linux(Type - 1), cho phép Linux biến thành một máy chủ ảo hóa để chạy nhiều máy ảo (VM). Nó cung cấp khả năng ảo hóa hỗ trợ phần cứng( Hardware-assisted Virtualization).
...
- **VirtIO Drivers**: Một driver đặc biệt để cải thiện hiệu xuất I/O (Networking và ổ đĩa) giữa khách hàng với máy chủ. Giảm chi phí mô phỏng, tăng tốc độ truy cập tài nguyên.
- **Libvirt**: Bộ công cụ và API dể quản lý ảo hóa, cho phép users start, stop và quản lý các máy ảo.
- **QEMU(Quick Emulator)**: Ứng dụng không gian người dùng mô phỏng phần cứng ảo( ổ cứng, mạng, GPU) cho máy ảo. Kết hợp với KVM, nó sử dụng tốc độ phần cứng cho CPU và bộ nhớ đồng thời mô phỏng các thiết bị khác như disk controllers và card mạng
