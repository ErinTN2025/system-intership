...
- `-A INPUT`: (Append to INPUT): Thêm quy tắc mới vào chuỗi INPUT. Chuỗi INPUT là chuỗi quản lý toàn bộ gói tin(traffic) đi từ bên ngoài vào máy chủ của bạn.
- `-p tcp`: (protocol): Chỉ định giao thức mạng là TCP. Nó bảo cho tường lửa biết quy tắc này chỉ áp dụng cho các gói tin truyền theo tiêu chuẩn TCP (không áp dụng cho UDP hay ICMP/ping).
- `-m conntrack` (Match module conntrack): Kích hoạt module theo dõi trạng thái kết nối (`conntrack`)
- `--ctstate NEW`(Connection tracking state): Chỉ áp dụng cho các kết nối mới. Đây là tham số đi kèm với -m conntrack ở trên. NEW nghĩa là gói tin này đang yêu cầu mở/khởi tạo một kết nối hoàn toàn mới (gói tin SYN trong bắt tay 3 bước của TCP).
- `-s` địa chỉ nguồn máy gửi
- `-d` địa chỉ ip máy đích
- `--dport` Cổng đích trên máy chủ 
- `-j` hành động sẽ làm: `ACCEPT`, `DROP`, `REJECT`

Ở trên có đoạn ta đã dùng `--ctstate ESTABLISHED, RELATED` :

```bash
# 1. Cho phép tất cả các gói tin thuộc kết nối ĐÃ MỞ TRƯỚC ĐÓ đi qua thật nhanh (Tối ưu hiệu năng)
iptables -A INPUT -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

# 2. Kiểm tra kỹ các kết nối MỚI: Chỉ cho phép mở kết nối mới vào các CỔNG CỤ THỂ
iptables -A INPUT -p tcp --dport 22 -m conntrack --ctstate NEW -j ACCEPT   # Cho phép SSH mới
iptables -A INPUT -p tcp --dport 443 -m conntrack --ctstate NEW -j ACCEPT  # Cho phép Web HTTPS mới

# 3. Chặn tất cả các kết nối MỚI vào các cổng còn lại (Security rule)
...
- Thực hiện cấu hình iptables trên máy chủ Firewall(iptables)
...
# Cho ping Webserver
iptables -A FORWARD -p icmp -s 192.168.122.0/24 -d 192.168.104.0/24 -j ACCEPT

# Cho HTTP
iptables -A FORWARD -p tcp -s 192.168.122.0/24 -d 192.168.104.0/24 --dport 80 -j ACCEPT

# Chặn SSH
iptables -A FORWARD -p tcp -s 192.168.122.0/24 --dport 22 -j DROP

# Chặn Internet
iptables -A FORWARD -s 192.168.122.0/24 -o ens33 -j DROP
...
# Cho ra Internet
iptables -A FORWARD -s 192.168.104.0/24 -o ens33 -j ACCEPT

# Cho ping Client
iptables -A FORWARD -p icmp -s 192.168.104.0/24 -d 192.168.122.0/24 -j ACCEPT

# Cho SSH sang Client
iptables -A FORWARD -p tcp -s 192.168.104.0/24 -d 192.168.122.0/24 --dport 22 -j ACCEPT

# ========================
# NAT
# ========================

iptables -t nat -A POSTROUTING -s 192.168.104.0/24 -o ens33 -j MASQUERADE
```
...
```
![altimage](../images/Screenshot_132.png)
- Chỉ có `libvirtd`, `192.168.104.0` và loopback -> OK

```bash
iptables -L -v -n
```

![altimage](../images/Screenshot_134.png)

- ACCEPT `.122` nhưng DROP `.104` -> OK
- ACCEPT `tcp` : 80 -> OK

- Test ok ping được tới Webserver, nhưng không ra được mạng

![altimage](../images/Screenshot_133.png)
