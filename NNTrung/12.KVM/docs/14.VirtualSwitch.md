...
- `Port`: là điểm kết nối của bridge - mỗi interface muốn tham gia vào bridge đều phải được gắn vào 1 port. Giống hệt cổng RJ-45 trên switch thật.
- `Tap interface`: là loại interface đặc biệt dành cho VM - nó hoạt động ở Layer 2, nằm trong kernel, và được QEMU/KVM dùng để kết nối VM vào bridge. Khi VM gửi Ethernet frame, frame đó ra khỏi tap và vào bridge ngay lập tức.
