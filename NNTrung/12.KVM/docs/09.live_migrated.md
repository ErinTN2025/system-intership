...
## 0. Linux Module
- Kernel module là một đoạn code có thể được nạp vào Linux kernel khi hệ thống đang chạy.
- Thay vì nhét tất cả chức năng vào kernel ngay từ lúc boot, Linux có thể để một số chức năng thành module, khi nào cần thì load vào.
- Check bằng `lsmod`
- Muốn load module ta dùng `modprobe`
    - Ví dụ:
```bash
sudo modprobe kvm_intel
```
- Xem kernel log liên quan module
```bash
dmesg | tail -50
```
- Xem module nào đang được load
```bash
lsmod | grep -E 'bridge|overlay|vxlan'
```
- Kernel đang chạy:
```bash
uname -r
...
### 2. Bản chất Live migration
- Di chuyển một VM đang chạy từ Host A sang Host B mà VM gần như không bị tắt.
- Điểm quan trọng là không phải copy file VM xong rồi chạy lại. Nó chủ yếu là copy trạng thái đang chạy của VM( RAM + CPU/device state) sang host mới.
...
- Lỗi này là do QEMU cũ và mới thường là do bạn dùng các phiên bản ubuntu khác nhau (không khuyến khích)
