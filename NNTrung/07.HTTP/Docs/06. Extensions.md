...
Tạo một Yêu cầu ký chứng chỉ - CSR (Certificate Signing Request) và lưu thành tệp `mydomain.csr`.

Mục đích chính của tệp CSR này là gửi cho một Cơ quan cấp chứng chỉ (CA) – hoặc tự bạn ký – để đổi lấy một chứng chỉ SSL/TLS chính thức cho địa chỉ IP 192.168.133.150.

- `openssl req`: Gọi công cụ quản lý và tạo yêu cầu chứng chỉ (Certificate Request) của OpenSSL.
- `new`: Yêu cầu tạo mới một CSR.
- `key server-key.pem`: Chỉ định khóa riêng tư bạn đã tạo ở bước trước. OpenSSL sẽ dùng khóa này để ký số lên CSR nhằm chứng minh bạn thực sự sở hữu khóa riêng tư đi kèm.
- `subj "/CN=192.168.133.150"`: Điền sẵn thông tin chủ thể cho chứng chỉ mà không cần bạn phải nhập thủ công từng dòng trên terminal.
  - CN viết tắt của Common Name (Tên chung). Đối với website thông thường, đây sẽ là tên miền (ví dụ: example.com). Trong trường hợp này, bạn đang cấu hình trực tiếp cho một địa chỉ IP nội bộ: 192.168.133.150. Khi tạo chứng chỉ cho một địa chỉ IP (thay vì tên miền), hầu hết các trình duyệt hiện đại (như Chrome, Edge) sẽ báo lỗi bảo mật nếu IP đó chỉ nằm ở trường CN (Common Name). Để trình duyệt chấp nhận hoàn toàn, địa chỉ IP cần phải được khai báo thêm trong phần SAN (Subject Alternative Name) của chứng chỉ.
- `out server.csr`: Chỉ định lưu yêu cầu ký chứng chỉ này vào tệp tin tên là server.csr.
...
- Hiểu đơn giản muốn được trust ta cần tối thiểu 3 thành phần: 1 client, 1 server và 1 trung gian đứng ra để trust cả 2 bên.

#### Tạo CA (Certificate Authority)
- Bước 1: Tạo private key của CA, bước này sẽ sinh ra `ca.key`
  - Đây là khóa bí mật của CA nếu bị hacker lấy được khóa này thì họ có quyền tạo ra chứng chỉ giả mạo.
  - Dùng để ký certificate cho người khác 
  - Không được để lộ
```bash
openssl genrsa -out ca.key 4096
```
- Bước 2: Tạo Certificate của CA
  - Đây sẽ là chứng chỉ công khai của CA
  - Mọi client sẽ cài `ca.crt` vào Trusted Store nếu muốn tin CA này. Tin CA này tương đương sau này chỉ cần CA này trust bất kỳ cái nào thì máy đó cũng sẽ tin theo.s
...
openssl req -x509 \
    ...
    -out ca.crt
```
- Như vậy tại CA sẽ có 2 file chính:
  - `ca.key`: để ký cert cho 1 bên khác
  - `ca.crt`: cert của CA, ai muốn tin CA thì trust cert này đồng nghĩa tin mọi thứ mà CA tin tưởng.

#### Giai đoạn 2: Máy chủ tạo khóa riêng
Giả sử máy LB muốn có https
- Bước 3: Tạo Private key của server:
  - Bước này sẽ sinh ra `lb.key`, đây là private key của Load Balancer.
  - Nó sẽ được Nginx hoặc HAProxy sử dụng để giải mã dữ liệu trong quá trình bắt tay TLS và chứng minh quyền sở hữu chứng chỉ.
  - Nếu để lộ Lb có thể bị người khác giả mạo.
```bash
openssl genrsa -out lb.key 2048
```
#### Giai đoạn 3: Xin cấp chứng chỉ
- Bước 4: Tạo CSR (Certificate Signing Request)
  - CSR sẽ chứa: Domain(`CN`,`SAN`), Public Key của `lb.key`, Thông tin tổ chức
  - CSR chỉ là một đơn xin cấp chứng chỉ
...
openssl req \
   ...
   -out lb.csr 
```
#### Giai đoạn 4: Chuẩn bị Extension
- Bước 5: Tạo file Extension
  - Được lưu thành `lb.ext`
  - File này nói với CA: Certificate này sẽ dùng làm Server Certificate
  - Ngoài ra còn khai báo: SAN, Key Usage, Extended Key Usage, Basic Constraints
  - File `.ext` là dữ liệu cấu hình bổ sung để CA sử dụng khi ký
#### Giai đoạn 5: CA ký chứng chỉ
- CA nhận `lb.csr`
- Nó sẽ dùng `ca.key`
- Sẽ sinh ra `lb.crt` chính là certificate của server
- Bên trong sẽ có Public Key, Domain, Thời hạn, Serial Number, chữ ký số của CA.
```bash
openssl x509 \
      -req \
      -in lb.csr \
...
#### Lưu ý quan trọng:
- File `lb.csr` khi gửi qua Internet cho CA trust hoàn toàn có thể bị lộ không sao cả, vì nó chỉ chứa Public key nên hoàn toàn có thể bị giả mạo hoặc đánh cắp thay đổi thông tin.
- Trong file csr có 1 cái chữ ký số Signature digital, chữ ký này được ký bằng chính private key của Load balancer. CA server sẽ lấy cái Public key trong CSR để kiểm tra chữ ký. Nếu kiểm tra thành công CA biết ng này chắc chắn là người sở hữu CSR.
- Vì vậy Client.key hay lb.key là thứ không được để lộ nếu để lộ sẽ bị giả mạo.
- Sau đó còn bước xác minh domain bạn gửi email hay file txt kèm theo v..v.

#### Giai đoạn 6: Tạo PEM (nếu cần)
- Một số phần mềm (như HAProxy) thích dùng PEM.
- Ví dụ:
```bash
cat lb.key lb.crt > lb.pem
```
- PEM chỉ là định dạng (Base64 với header/footer), không phải một loại chứng chỉ mới. Trong ví dụ này, `lb.pem` đơn giản là file gộp giữa `lb.key` và `lb.crt`.
