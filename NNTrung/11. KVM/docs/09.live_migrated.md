...
|----------|--------------|----|-------|--------|-----|
|NFS Server| Ubuntu 24.04 | 192.168.148.134| 2 | 6 | 20 |
|KVM01| Ubuntu 24.04 | 192.168.148.137| 2 | 6 | 20 |
|KVM02| Ubuntu 24.04 | 192.168.148.138| 2 | 6 | 20 |
...
```bash
/root/storage 192.168.148.137/24(rw,sync,no_root_squash)
/root/storage 192.168.148.138/24(rw,sync,no_root_squash)
```
Cập nhập lại file 
```bash
exportfs -a
...
```bash
sudo mount 192.168.148.134:/root/storage/  storage
```
hoặc khi sử dụng thư mục mặc định chứa disk của Vm
```bash
sudo mount 192.168.174.135:/root/storage/ /var/lib/libvirt/images/
```
Mount tự động khi khởi động:
```bash
echo "192.168.148.134:/root/storage/ /var/lib/libvirt/images/ nfs defaults 0 0" | sudo tee -a /etc/fstab
```
### 3. Cài đặt KVM
