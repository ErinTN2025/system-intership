...
- **Chassis(khung/máy chủ vật lý)**: một node vật lý (hoặc VM host) đang chạy OVS (Open vSwitch) và tham gia vào mạng OVN.
  - Hãy hình dung: bạn có 1 cụm gồm 10 server vật lý, mỗi server chạy nhiều VM/container. Mỗi server đó, trong OVN, được gọi là 1 Chassis.
  - **Chassis registration** (đăng ký chassis) = khi 1 server khởi động `ovn-controller` (tiến trình chạy trên từng node để giao tiếp với OVN), nó sẽ tự đăng ký chính mình vào SBDB, báo cáo: "Tôi là chassis tên X, tôi có địa chỉ IP là Y, tôi hỗ trợ các loại tunnel encapsulation Z (geneve, vxlan...)".
- **Tunnel Endpoints**: Vì VM/container trên các chassis khác nhau cần gửi traffic cho nhau qua mạng vật lý thật (physical network), OVN dùng kỹ thuật tunneling (đóng gói) — biến traffic mạng ảo (overlay) thành gói tin có thể truyền qua mạng vật lý (underlay).
- **Datapath Bindings**: Datapath trong OVN = một mạng logic (logical switch hoặc logical router) — tức là 1 đơn vị mạng ảo mà bạn định nghĩa ở NBDB (VD: "logical switch tên ls1").
  - việc ánh xạ (map) 1 datapath logic đó với 1 mã định danh cụ thể (tunnel key) mà tất cả các chassis dùng chung để nhận diện nó khi truyền dữ liệu qua tunnel.
- **Port binding** = thông tin ánh xạ giữa 1 cổng logic (logical port) — đại diện cho 1 VM, container, hoặc interface ảo — với chassis vật lý nơi nó đang thực sự chạy.
- **Flow Information**: Đây là bước cuối cùng, cụ thể nhất — nơi mọi thông tin trên (chassis, tunnel, datapath, port) được dịch thành luật cụ thể để OVS (Open vSwitch) trên từng chassis biết cách xử lý từng gói tin.
  - Flow (luồng) ở đây thực chất là OpenFlow rules — quy tắc kiểu "if-then" cực kỳ chi tiết mà OVS dùng để quyết định số phận từng gói tin đi qua nó. Ví dụ 1 flow đơn giản (viết theo dạng dễ hiểu, không phải cú pháp thật).

1. Chassis: "Tôi là server nào, tôi tồn tại"
2. Tunnel endpoint: "Địa chỉ vật lý của tôi để người khác gửi gói tin đến"
3. Datapath binding: "Mạng ảo X được gán mã số Y để nhận diện qua tunnel"
4. Port binding: "VM/container Z hiện đang chạy trên chassis nào"
5. Flow: "Quy tắc cụ thể: gói tin như thế này thì xử lý/chuyển tiếp như thế kia"
