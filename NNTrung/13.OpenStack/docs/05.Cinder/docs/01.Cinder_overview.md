...
- Bước 1: (1) Người dùng gửi yêu cầu: Người dùng (Client/Admin) gửi một lệnh tạo backup (ví dụ: `openstack volume backup create`) tới `cinder-api` thông qua cổng giao tiếp REST API.

- Bước 2: Đọc/Xác thực yêu cầu: `cinder-api` tiếp nhận request, kiểm tra quyền hạn (authentication/authorization) và lưu thông tin khởi tạo bản ghi backup vào cơ sở dữ liệu (SQL).

- Bước 3: Đẩy task vào Queue: cinder-api gửi một thông điệp (message) yêu cầu thực hiện sao lưu vào hệ thống hàng đợi Messaging Bus (AMQP - RabbitMQ). cinder-backup sẽ nhận thông điệp này từ Queue.

- Bước 4: Khởi tạo tiến trình Backup: cinder-backup kết hợp với cinder-volume tương ứng để chuẩn bị truy xuất dữ liệu từ volume gốc.

- Bước 5: Đọc dữ liệu gốc: cinder-volume kết nối tới thiết bị lưu trữ gốc (Backend storage - ví dụ: NetApp/Ceph/LVM) để đọc block dữ liệu của ổ đĩa cần backup.

- Bước 6: Ghi dữ liệu vào nơi lưu trữ Backup: cinder-backup ghi/chép trực tiếp dữ liệu vừa đọc được vào repository sao lưu đích (ở đây mô tả là NFS Backup Repository).

- Bước 7: Báo cáo hoàn tất task: Sau khi dữ liệu được sao lưu hoàn tất vào kho chứa, cinder-backup gửi thông điệp phản hồi kết quả thành công về lại Messaging Bus (AMQP) và cập nhật trạng thái trong SQL.

- Bước 8: Nhận phản hồi: cinder-api lắng nghe thông điệp hoàn thành từ Messaging Bus.

- Bước 9: Trả kết quả cho User: cinder-api gửi phản hồi HTTP (kèm thông tin bản backup vừa tạo thành công) về cho Người dùng.
