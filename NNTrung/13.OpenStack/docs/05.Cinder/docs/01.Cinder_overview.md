...
- **Tính năng nâng cao**: Hỗ trợ tạo Snapshot (ảnh chụp tức thời), Clone (nhân bản), Backup, Encryption, Multi-attach và Migration giữa các backend.
...
- **Hot-plug vào Hypervisor**:
  ...
  - Hypervisor thực hiện gắn đĩa "nóng" vào máy ảo. Máy ảo sẽ nhìn thấy một thiết bị mới (ví dụ: `/dev/vdb`).
...
Người dùng (hoặc thông qua giao diện Horizon/CLI) gửi một yêu cầu HTTP REST API để tạo bản backup cho một volume cụ thể. Yêu cầu này được gửi tới cổng tiếp nhận của Cinder.

- Bước 2: `cinder-api` tiếp nhận và xác thực

Thành phần `cinder-api` nhận yêu cầu từ bước 1. Nó sẽ kiểm tra quyền hạn của người dùng (qua Keystone), xác thực thông tin và ghi nhận trạng thái yêu cầu vào cơ sở dữ liệu (**SQL**). Sau đó, nó đẩy một tin nhắn (message) chứa yêu cầu này vào hàng đợi **Messaging Bus (AMQP/RabbitMQ)** để điều phối xuống các dịch vụ bên dưới.

- Bước 3: `cinder-backup` tiếp nhận nhiệm vụ

Thành phần `cinder-backup` liên tục "nghe" tín hiệu từ Messaging Bus. Khi thấy có yêu cầu tạo backup, nó sẽ bốc tin nhắn này về để xử lý và bắt đầu quá trình phối hợp với các driver lưu trữ.

- Bước 4: Khởi tạo và liên kết với `cinder-volume`

Dịch vụ `cinder-backup` tương tác với dịch vụ `cinder-volume` (nơi đang trực tiếp quản lý phân vùng ổ cứng/Backend lưu trữ của máy ảo) để xác định vị trí chính xác của Volume cần backup và chuẩn bị luồng đọc dữ liệu.

- Bước 5: Đọc dữ liệu từ Volume gốc (Backend)

Hệ thống bắt đầu đọc dữ liệu thô (gốc) từ thiết bị lưu trữ đang chứa Volume của máy ảo (ở đây sơ đồ minh họa bằng kết nối tới cụm lưu trữ sản xuất - Production Storage).

- Bước 6: Ghi dữ liệu vào kho lưu trữ Backup (NFS Backup Repository)

Song song với việc đọc dữ liệu ở bước 5, dịch vụ `cinder-backup` tiến hành chuyển đổi và ghi trực tiếp các luồng dữ liệu này vào kho lưu trữ backup an toàn được cấu hình sẵn (sơ đồ hiển thị là hệ thống **NFS Backup Repository**).

- Bước 7: Báo cáo hoàn thành tác vụ lên Message Bus

Sau khi quá trình đọc-ghi dữ liệu backup kết thúc thành công, `cinder-backup` gửi một tin nhắn phản hồi ngược lại lên **Messaging Bus (AMQP)** để thông báo rằng: *"Tôi đã hoàn thành việc tạo backup ổn định"*. Trạng thái của bản backup trong cơ sở dữ liệu **SQL** cũng được cập nhật từ `creating` sang `available`.

- Bước 8: `cinder-api` nhận kết quả thành công

`cinder-api` nhận được tin nhắn thông báo hoàn thành từ Message Bus. Nó nhận biết được tác vụ đã kết thúc tốt đẹp và chuẩn bị phản hồi cho người dùng.

- Bước 9: Trả kết quả về cho người dùng

`cinder-api` gửi phản hồi HTTP thành công (Success Response) về cho người dùng. Lúc này trên giao diện hoặc câu lệnh CLI, người dùng sẽ nhìn thấy bản Backup của mình đã xuất hiện với trạng thái sẵn sàng sử dụng.
...
## 6. Không có Cinder vẫn có thể tạo được máy ảo
Trong OpenStack:
