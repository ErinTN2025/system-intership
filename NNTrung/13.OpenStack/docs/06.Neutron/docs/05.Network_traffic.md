#  Mạng ảo OpenStack hoạt động thế nào 
...
- Muốn cấp IP tự động (DHCP)? → có `neutron-dhcp-agent`, nó phát sinh ra một namespace `qdhcp-xxx`, bên trong chạy một con `dnsmasq`.
- Muốn định tuyến giữa các mạng (L3)? → có `neutron-l3-agent`, cần thên ra namespace `qrouter-xxx`, bên trong dùng `iptables` để NAT.
- Muốn máy ảo lấy được metadata (như SSH key)? → có `neutron-metadata-agent`, lại thêm một `haproxy` trong namespace nữa.
