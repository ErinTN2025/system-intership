...
# Không cài/test Tempest khi dựng lab
INSTALL_TEMPEST=False

# Nếu DevStack chạy trong VMware và chưa nested virtualization:
LIBVIRT_TYPE=qemu
# Nếu đã chắc chắn nested virtualization bật tốt, đổi thành:
# LIBVIRT_TYPE=kvm
...
# OVN không dùng các Neutron agent cũ
disable_service q-agt
disable_service q-l3
disable_service q-dhcp
disable_service q-meta

# Extension Neutron dùng tốt trong lab
...
# One-node này đồng thời là gateway chassis
ENABLE_CHASSIS_AS_GW=True

# -------------------------
# Tenant and Floating IP ranges
# Không trùng mạng host 192.168.207.0/24
# -------------------------
IP_VERSION=4
...
Q_FLOATING_ALLOCATION_POOL=start=172.24.5.100,end=172.24.5.200
...
SWIFT_REPLICAS=1
SWIFT_LOOPBACK_DISK_SIZE=5G
EOF
```
- Nếu lỡ xóa thư mục Neutron do lỗi phiên bản hoặc xóa bất cứ j đó thì cứ dựa vào đây mà khôi phục:
```bash
test -d /opt/stack/neutron/.git || \
git clone -b stable/2026.1 https://opendev.org/openstack/neutron.git /opt/stack/neutron
```

- Kiểm tra 3 repo chính phải cùng branch:
```bash
git -C /opt/stack/devstack branch --show-current
git -C /opt/stack/neutron branch --show-current
git -C /opt/stack/requirements branch --show-current
```
