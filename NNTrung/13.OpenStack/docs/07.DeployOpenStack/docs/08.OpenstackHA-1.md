...
- Giai đoạn 3: HAProxy + Keepalived (VIP .120)
- Giai đoạn 4: Keystone
- Giai đoạn 4.5: Ceph cluster
- Giai đoạn 5: Glance HA + Ceph backend
- Giai đoạn 6: Nova
- Giai đoạn 7: Neutron / OVN (control + data plane)
- Giai đoạn 8: Cinder multi-backend (LVM + NFS + Ceph RBD qua volume types)
- Giai đoạn 9: UI Horizon + kiểm thử HA tổngss
...
```bash
sudo tee /etc/chrony/chrony.conf > /dev/null <<'EOF'
server controller iburst prefer
...
**VRRP** là viết tắt của cụm từ **Virtual Router Redundancy Protocol** (Giao thức dự phòng bộ định tuyến ảo).

Đây là một giao thức mạng tiêu chuẩn (được định nghĩa trong RFC 5798) nhằm tăng tính sẵn sàng và khả năng chịu lỗi cho hệ thống mạng.

#### Cơ chế hoạt động cơ bản của VRRP:

* **Nhóm các Router thành một nhóm ảo:** VRRP cho phép kết hợp nhiều thiết bị định tuyến (Router hoặc Switch Layer 3, hoặc các máy chủ chạy Keepalived như bạn cấu hình ở trên) lại với nhau để cùng chia sẻ một địa chỉ IP ảo (Virtual IP - VIP) đóng vai trò làm Default Gateway cho người dùng.
* **Cơ chế Master - Backup:** Trong nhóm này, tại một thời điểm chỉ có một thiết bị làm **Master** (chịu trách nhiệm xử lý gói tin), các thiết bị còn lại sẽ làm **Backup** (đứng đợi ở chế độ dự phòng).
* **Tự động chuyển mạch (Failover):** Nếu thiết bị Master bị sập hoặc mất kết nối, thiết bị Backup có điểm ưu tiên (`priority`) cao nhất sẽ lập tức đứng lên thay thế, tiếp quản địa chỉ VIP đó mà người dùng hoặc các ứng dụng phía sau không hề hay biết, giúp hệ thống không bị gián đoạn.

Đây là file cấu hình của **Keepalived** – công cụ chịu trách nhiệm quản lý **VIP (Virtual IP)** và đảm bảo tính sống còn (High Availability - HA) cho hệ thống OpenStack của bạn.

Về cơ bản, cấu hình này định nghĩa node hiện tại (`controller01`) đóng vai trò là **MASTER** (node chính) nắm giữ IP ảo `192.168.70.120`. Nó cũng liên tục giám sát xem HAProxy có còn sống không để tự động chuyển giao IP này cho node khác nếu có sự cố.

Dưới đây là giải thích chi tiết từng khối lệnh:

---

#### Khối định danh (`global_defs`)
...
* **`router_id controller01`**: Đặt tên định danh cho node này trong cụm cluster là `controller01`. Tên này giúp bạn dễ phân biệt khi đọc log hệ thống.

---

####  Khối giám sát dịch vụ (`vrrp_track_process`)
...
Khối này tạo ra một bộ giám sát (tracker) có tên là `chk_haproxy`:

* **`process haproxy`**: Bảo Keepalived theo dõi tiến trình (process) có tên là `haproxy`.
* **`weight -30`**: Đây là đòn "phạt" điểm. Nếu HAProxy bị chết, điểm ưu tiên (`priority`) của node này sẽ **bị trừ đi 30 điểm**.
* **`delay 2`**: Khoảng thời gian (tính bằng giây) để kiểm tra lại sau khi trạng thái thay đổi.
* **`fall 2` / `rise 2**`: Phải kiểm tra thất bại **2 lần liên tiếp** thì mới coi là HAProxy chết hẳn (và trừ điểm), và ngược lại phải thành công **2 lần liên tiếp** mới coi là đã hồi phục.


#### Khối cấu hình IP ảo (`vrrp_instance`)

Khối này định nghĩa một cụm HA chạy giao thức VRRP tên là `VI_OPENSTACK`.

* **Cấu hình vai trò và kết nối:**
```syslog
state MASTER
interface eth0
virtual_router_id 70
...
* `state MASTER`: Khai báo ban đầu node này là **Master** (Node chính giữ VIP).
* `interface eth0`: Cạc mạng vật lý mà Keepalived sẽ gán VIP lên và dùng để giao tiếp với các node khác.
* `virtual_router_id 70`: Mã định danh của cụm VRRP này. **Tất cả các node trong cụm (Master và Backup) phải dùng chung một số này** (ở đây là số 70).
* `priority 110`: Điểm số ưu tiên của node này là **110**. Node nào có điểm cao nhất sẽ được làm Master. (Node Backup thường sẽ cấu hình điểm thấp hơn, ví dụ: 100).
* `advert_int 1`: Cứ mỗi **1 giây**, Master sẽ gửi một gói tin (heartbeat) để báo cho các node Backup biết là *"Tao vẫn còn sống"*.
...
* Xác thực giữa các node trong cụm bằng mật khẩu (`PASS`) với password là `OpenStac`. Các node muốn kết nối với nhau phải khớp mật khẩu này để tránh việc có máy lạ nhảy vào phá cụm.


* **Cấu hình mạng Unicast (`unicast_src_ip` & `unicast_peer`):**
```syslog
unicast_src_ip 192.168.70.122
unicast_peer {
    192.168.70.127
...
* Mặc định Keepalived dùng Multicast, nhưng cấu hình này đang ép nó dùng **Unicast** (gửi gói tin trực tiếp).
* `unicast_src_ip`: IP của chính node này (`192.168.70.122`).
* `unicast_peer`: Danh sách IP của các node Backup còn lại trong cụm (`.127` và `.119`). Nghĩa là cụm OpenStack này của bạn có tổng cộng **3 node**.


* **Khai báo IP ảo (`virtual_ipaddress`):**
```syslog
virtual_ipaddress {
    192.168.70.120/24 dev eth0
}
...
* Đây chính là địa chỉ **VIP** (`192.168.70.120`). Địa chỉ này sẽ tự động được gán vào cạc `eth0` nếu node này chiếm được quyền MASTER.


* **Gắn bộ giám sát vào cụm (`track_process`):**
```syslog
track_process {
    chk_haproxy
}
...
* Kích hoạt bộ giám sát `chk_haproxy` (đã định nghĩa ở mục 2) cho instance này.



---

#### Kịch bản vận hành thực tế của file này sẽ như thế nào?

1. **Trạng thái bình thường:** Node này khởi động lên, thấy mình là `MASTER` và có điểm `110` (cao nhất cụm). Nó sẽ giật lấy VIP `192.168.70.120` để tiếp nhận các yêu cầu gửi đến OpenStack.
2. **Khi HAProxy gặp sự cố:** Bộ giám sát phát hiện HAProxy trên node này bị chết $\rightarrow$ Kích hoạt `weight -30` $\rightarrow$ Điểm ưu tiên bị trừ: $110 - 30 = 80$.
3. **Chuyển giao quyền lực (Failover):** Lúc này điểm của node Master chỉ còn `80`, thấp hơn điểm của node Backup (thường là `100`). Node Backup thấy vậy sẽ lập tức giật lấy VIP `192.168.70.120`. Hệ thống OpenStack của bạn vẫn hoạt động bình thường mà không bị gián đoạn.
