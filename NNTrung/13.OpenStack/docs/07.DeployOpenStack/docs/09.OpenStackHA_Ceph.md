...
> Lệnh trên xoá vĩnh viễn `vdb`. Chắc chắn là vdb (40G), KHÔNG phải vda (OS).
...
> **`ceph.pub` là PUBLIC key**, không phải private. Cephadm giữ private key trong
...
> **Vì sao không dùng `ceph orch apply osd --all-available-devices`?** Lệnh đó tạo spec
> declarative "mọi đĩa trống ở mọi host = OSD" — tiện nhưng tương lai thêm `vdc` sẽ bị ăn
> ngầm. Trong lab biết rõ 3 OSD cần dựng → dùng `daemon add` rõ ràng hơn, dễ debug, dễ gỡ.
...
## 6* Setup deployer làm admin node
- lưu ý làm đoạn này thì không cần làm đoạn cuối, mục đích đoạn này chỉ để test
### 1. Cài `ceph-common` trên deployer
```bash
# trên deployer
sudo apt update
...
ceph --version
```
### 2. Copy `ceph.conf` + admin keyring từ ceph01 sang deployer
```bash
# trên ceph01 — tạo bản tạm readable
...
```
```bash
# trên deployer — pull về đặt đúng chỗ
sudo mkdir -p /etc/ceph
sudo scp ceph01:/tmp/ceph.conf /etc/ceph/
sudo scp ceph01:/tmp/ceph.client.admin.keyring /etc/ceph/
sudo chmod 600 /etc/ceph/ceph.client.admin.keyring
sudo chmod 644 /etc/ceph/ceph.conf
ls -l /etc/ceph/
```
```bash
# trên ceph01 — DỌN FILE TẠM (keyring quyền god-mode, không để lung tung)
...
> **KHÔNG tắt 2 ceph node cùng lúc** — mất quorum mon, mất min_size 2, cụm đứng. Cẩn thận
...
# GIAI ĐOẠN 5 — GLANCE (Image Service) HA + BACKEND CEPH RBD

> **Tiền đề:** Giai đoạn 4 xanh (Keystone HA qua VIP `.120`), Giai đoạn 4.5 xanh (Ceph cluster
> `HEALTH_OK`, pool `images` đã tạo + `application rbd`). Deployer có sẵn `ceph-common` + admin
> keyring (gõ `ceph -s` ra trạng thái cluster).
> **Phạm vi:** Glance-API chạy trên cả 3 controller, sau HAProxy/VIP, lưu image **trực tiếp
> trong pool `images` của Ceph** (không lưu file local).
> **Mục tiêu:** `openstack image create` upload cirros → image hiện trong `rbd ls images`,
> `openstack image list` trả về qua VIP, tắt 1 controller vẫn upload/list được.

---

## 0. GLANCE LÀ GÌ + TẠI SAO CEPH BACKEND

**Glance = "kho ảnh khởi tạo VM".** Mỗi VM được tạo từ 1 image (Ubuntu cloud, CentOS, cirros…).
Glance giữ ảnh + metadata; Nova khi tạo VM sẽ hỏi Glance lấy image rồi bung thành disk VM.

**Backend storage của Glance** quyết định image lưu ở đâu:
- `file` — đĩa local của node Glance. Đơn giản, nhưng không HA: image upload vào controller01
  thì controller02/03 không có.
- `swift` — Object storage. HA tự nhiên nhưng phải dựng Swift trước (chưa có).
- **`rbd` (Ceph)** — image là RBD object trong pool. **Cả 3 controller cùng đọc/ghi 1 chỗ**,
  HA miễn phí, copy-on-write khi tạo VM trên Nova+Ceph (Giai đoạn 6) → tạo VM cực nhanh.

→ Chọn `rbd`. Đây là pattern chuẩn cho mọi cloud OpenStack có Ceph.

**Cơ chế xác thực Ceph (cephx):**
Mỗi client Ceph có 1 keyring + caps (quyền) riêng. Ta sẽ tạo `client.glance` với caps hẹp:
- `mon 'profile rbd'` — chỉ đủ để biết bản đồ cụm + tìm object.
- `osd 'profile rbd pool=images'` — chỉ đọc/ghi pool `images`, không đụng pool khác.

→ Nếu kẻ xấu lấy được glance.keyring, hắn chỉ phá được pool `images`, không phá được `volumes`
(của Cinder) hay `vms` (của Nova). Nguyên tắc least-privilege.

**3 thứ Glance cần để chạy:**
1. **DB `glance`** trong MariaDB (lưu metadata image: tên, size, owner, status).
2. **Keystone integration** (user `glance` + service + 3 endpoint trỏ VIP).
3. **Ceph access** (`/etc/ceph/ceph.conf` + `ceph.client.glance.keyring`).

---

## 1. TẠO CEPH USER `client.glance` (trên deployer, có admin keyring)
...
# xem nội dung
sudo cat /tmp/ceph.client.glance.keyring
```
Mong đợi:
```
[client.glance]
...
> File `ceph.client.glance.keyring` rất nhạy cảm — ai cầm nó ghi đè được mọi image trong pool
> `images`. Quyền file phải `600`, owner `glance:glance` ở controller.

---

## 2. PHÂN PHỐI `ceph.conf` + `glance.keyring` SANG 3 CONTROLLER

Controller cần 2 file để dùng Ceph: `/etc/ceph/ceph.conf` (địa chỉ mon) + keyring của user
`client.glance`.

```bash
# trên deployer — copy sang controllers
for h in controller01 controller02 controller03; do
  ...
  sudo scp /tmp/ceph.client.glance.keyring root@$h:/etc/ceph/ceph.client.glance.keyring
done

# trên deployer — xoá file tạm
sudo rm /tmp/ceph.client.glance.keyring
```

> Lưu ý: `sudo scp` từ deployer hoạt động vì root@deployer có SSH key sang root@controllerXX.
> Nếu prompt password → có vấn đề SSH, kiểm tra lại.

Verify trên 1 controller:
```bash
ssh controller01 'sudo ls -l /etc/ceph/'
# phải có ceph.conf và ceph.client.glance.keyring
```

---

## 3. CÀI `ceph-common` + `glance-api` TRÊN 3 CONTROLLER

```bash
# từ deployer chạy song song trên 3 node
for h in controller01 controller02 controller03; do
  echo "=== $h ==="
  ssh $h 'sudo apt update && sudo apt -y install ceph-common glance-api python3-rbd python3-rados'
done
```

> - `ceph-common`: CLI `ceph`, `rbd`, `rados` + lib client.
> - `glance-api`: service Glance chính.
> - `python3-rbd` + `python3-rados`: binding Python để `glance_store` driver `rbd` dùng được.
> Thiếu 2 gói python này = upload image sẽ báo `glance_store driver rbd not available`.

### 3.1. Set owner cho glance keyring (TRÊN MỖI CONTROLLER)

Glance daemon chạy bằng user `glance`. Keyring phải đọc được bởi user này:
...
> `chmod 640` (group đọc) thay vì `600` (chỉ owner đọc) vì owner đang là `root`. Group =
> `glance` → daemon đọc được, user khác không.

### 3.2. Test client.glance từ controller (verify trước khi cấu hình glance.conf)

```bash
ssh controller01 'sudo -u glance rbd -n client.glance --keyring=/etc/ceph/ceph.client.glance.keyring -p images ls'
# mong đợi: không lỗi, return rỗng (pool images chưa có image)
```

Nếu báo `connect to mon … failed` → ceph.conf sai hoặc network không thông tới ceph01/02/03.
Nếu báo `auth ... unauthorized` → keyring sai hoặc caps sai.

---

## 4. TẠO DATABASE `glance` (1 lần, từ controller bất kỳ)
...
> Đổi `GLANCE_DBPASS` thành mật khẩu thật. **GHI LẠI**. User `'glance'@'%'` (không phải
> `'localhost'`) — cùng lý do với Keystone: connect qua VIP.

Verify Galera sync:
```bash
ssh controller02 'sudo mariadb -e "SHOW DATABASES;" | grep glance'
```

---

## 5. KEYSTONE: TẠO USER + SERVICE + ENDPOINT CHO GLANCE (từ deployer)

```bash
# trên deployer
source ~/admin-openrc

# 5.1 User glance trong project 'service'
...
# 5.3 Ba endpoint (public/internal/admin), TẤT CẢ trỏ VIP cổng 9292
openstack endpoint create --region RegionOne image public   http://192.168.70.120:9292
openstack endpoint create --region RegionOne image internal http://192.168.70.120:9292
openstack endpoint create --region RegionOne image admin    http://192.168.70.120:9292

# verify
openstack endpoint list --service image
```

> Đổi `GLANCE_PASS` thành mật khẩu thật. **GHI LẠI** — sẽ dùng trong `glance-api.conf`.
...
## 6. CẤU HÌNH `/etc/glance/glance-api.conf` (3 NODE — bind_host KHÁC NHAU)

Phải bind cụ thể IP node để khỏi chiếm cổng 9292 trên VIP (lặp lại bài học từ Keystone Apache
ở Giai đoạn 4).

### 6.1. Backup gốc + tạo config mới (TRÊN MỖI CONTROLLER, sửa IP cho phù hợp)

**controller01** (`bind_host = 192.168.70.122`):
```bash
ssh controller01 "sudo cp /etc/glance/glance-api.conf /etc/glance/glance-api.conf.bak"

ssh controller01 "sudo tee /etc/glance/glance-api.conf > /dev/null <<'EOF'
[DEFAULT]
...
[database]
connection = mysql+pymysql://glance:GLANCE_DBPASS@192.168.70.120/glance
max_retries = -1
...
**controller02** — copy hệt nhưng đổi `bind_host = 192.168.70.127`.
**controller03** — `bind_host = 192.168.70.119`.

> **Hạn chế `nano`** vào file này. Comment tiếng Việt có dấu sẽ làm
> `UnicodeDecodeError` lúc khởi động Glance (bài học từ Giai đoạn 4 Keystone). Dùng `tee`
> heredoc + ASCII only.

> Thay tất cả `GLANCE_DBPASS`, `GLANCE_PASS`, `RABBIT_PASS` bằng giá trị thật.

### 6.2. Sửa quyền file config

Glance sinh ra `glance-api.conf` mode 640, owner root:glance — `tee` ghi đè làm mất quyền. Sửa lại:
```bash
for h in controller01 controller02 controller03; do
  ssh $h '
...
## 7. SYNC SCHEMA DB (1 lần, trên controller01)

```bash
ssh controller01 'sudo -u glance glance-manage db_sync'
```

Mong đợi: in ra "Database is synced successfully." (có thể có WARNING về deprecation — kệ).

Verify từ controller02:
```bash
ssh controller02 'sudo mariadb glance -e "SHOW TABLES;" | head -20'
```
Phải thấy nhiều bảng (`images`, `image_locations`, `image_members`, `tasks`...).

---

## 8. KHỞI ĐỘNG GLANCE-API TRÊN 3 NODE
...
# verify nghe đúng IP node
for h in controller01 controller02 controller03; do
  echo "=== $h ==="
  ssh $h 'sudo ss -lntp | grep 9292'
done
```

Mong đợi mỗi node thấy 1 dòng `LISTEN ... <IP_NODE>:9292 ... users:(("glance-api"...))`.

Nếu fail:
```bash
ssh controller01 'sudo journalctl -u glance-api -n 50 --no-pager'
```
Lỗi hay gặp:
- `Could not connect to MariaDB` → DB pass sai hoặc Galera/VIP chưa thông.
- `glance_store driver rbd not available` → thiếu `python3-rbd`/`python3-rados` (quay lại 3.1).
- `Permission denied: keyring` → quyền keyring sai (quay lại 3.1).
- `UnicodeDecodeError` → có ký tự non-ASCII trong config, viết lại bằng heredoc ASCII.

---

## 9. THÊM GLANCE VÀO HAPROXY (trên cả 3 controller)
...
# === Glance (Image API) ===
listen glance_api
    bind 192.168.70.120:9292
    mode tcp
    balance roundrobin
    option tcp-check
    tcp-check connect
    server controller01 192.168.70.122:9292 check inter 2s rise 2 fall 3
    server controller02 192.168.70.127:9292 check inter 2s rise 2 fall 3
    server controller03 192.168.70.119:9292 check inter 2s rise 2 fall 3
EOF"
...
```bash
curl -s 'http://192.168.70.120:9000/stats;csv' -u admin:HAProxyStatsPass | grep glance_api | awk -F, '{print $1, $2, $18}'
# mong đợi:
# glance_api FRONTEND OPEN
# glance_api controller01 UP
# glance_api controller02 UP
# glance_api controller03 UP
# glance_api BACKEND UP
```
...
## 10. VERIFY GIAI ĐOẠN 5

### 10.1. List image qua VIP (lúc này rỗng)
...
### 10.2. Upload cirros + check image trong Ceph
...
Mong đợi:
- Image status `active`.
- `disk_format qcow2`, `size ~21MB`.
- `direct_url rbd://<fsid>/images/<image-id>/snap` ← **THIS LÀ DẤU HIỆU CEPH BACKEND HOẠT ĐỘNG**.

![altimage](../images/Screenshot_31.png)

### 10.3. Verify image thật sự nằm trong Ceph

```bash
# trên deployer (đã có ceph-common + admin keyring)
sudo rbd -p images ls
# mong đợi: 1 dòng image ID (uuid)

sudo rbd -p images info <image-id-từ-output-trên>
# size 21 MiB, format 2, có snap 'snap'
```

→ Image đã nằm trong Ceph pool `images`, replica 3 ở 3 OSD. Glance chỉ giữ metadata, file
binary sống trong Ceph.

![altimage](../images/Screenshot_32.png)

### 10.4. HA Test (gộp luôn Test B Keystone treo từ Giai đoạn 4)

**Test A — tắt glance-api ở controller01:**
```bash
ssh controller01 'sudo systemctl stop glance-api'
openstack image list                           # vẫn ra cirros
openstack image show cirros-0.6.2              # vẫn được
ssh controller01 'sudo systemctl start glance-api'
```

**Test B — tắt nguyên controller01 (đây là test failover full stack, defer từ Phase 4):**
```bash
ssh controller01 'sudo shutdown -h now'
sleep 30

# trên deployer — verify VIP nhảy + service vẫn sống
ping -c 3 192.168.70.120                       # vẫn ping được (VIP nhảy sang 02 hoặc 03)
openstack token issue                          # vẫn ra token (Keystone HA)
openstack image list                           # vẫn ra cirros (Glance HA)

# bật lại controller01
# trên cloud console: power on controller01
# đợi 2-3 phút cho Galera join, các service auto-start
ssh controller01 'sudo ceph -s 2>&1 || true; sudo systemctl is-active mariadb glance-api haproxy keepalived'
```

Sau khi controller01 lên lại, verify VIP có "fall back" về controller01 (vì priority 110) hay
vẫn ở node đang giữ — đều OK miễn 1 node giữ. Test fall back:
```bash
for h in controller01 controller02 controller03; do
  echo "=== $h ==="
  ssh $h "ip -br addr show eth0 | grep 192.168.70.120 || echo 'no VIP'"
done
```

### 10.5. Test upload lúc 1 controller chết

```bash
ssh controller02 'sudo systemctl stop glance-api'   # tắt 1 backend Glance
openstack image create "test-ha" --file cirros-0.6.2-x86_64-disk.img --disk-format qcow2 --container-format bare --public
# vẫn upload được (HAProxy gửi sang 01 hoặc 03)
openstack image delete test-ha
ssh controller02 'sudo systemctl start glance-api'
```
...
- Glance Install — Ubuntu (2025.1):
  https://docs.openstack.org/glance/2025.1/install/install-ubuntu.html
- Glance + Ceph RBD backend:
  https://docs.ceph.com/en/squid/rbd/rbd-openstack/#configuring-glance
- glance_store config reference (rbd):
  https://docs.openstack.org/glance_store/latest/configuration/configuring.html
