...
## CRUD in file sudo crudini --set <file_cấu_hình> <tên_section> <tên_key> <giá_trị>
...
openstack server show vm-lab -f value -c OS-EXT-SRV-ATTR:host
```
- Lấy toàn bộ thông tin chi tiết máy ảo có tên vm-lab
- `-c`: viết tắt của Column
  - `OS-EXT-SRV-ATTR:host` là trường thông tin mở rộng của Nova (OpenStack Compute) lưu giữ thông tin tên host vật lý đang chứa máy ảo đó.
- `-f`: khóa địng dạng đầu ra 
```bash
