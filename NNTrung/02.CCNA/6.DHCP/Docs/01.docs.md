...
- Client sau đó chuyển sang trạng thái BOUND và cấu hình giao diện:
  ...
  - Gửi ARP probe/gratuitous ARP để kiểm tra trùng IP (nếu thấy trùng sẽ gửi **DHCP DECLINE** để từ chối và xin lại).
    - Để gửi một gói tin ARP Probe hoặc Gratuitous ARP ra ngoài mạng mạng nhằm kiểm tra xem "Có ai đang dùng IP này không?", card mạng của Client bắt buộc phải có một IP nguồn (Source IP) hợp lệ nằm trong dải mạng đó để điền vào phần Header của gói tin.
    - Nếu Client chưa cấu hình IP và Subnet Mask vào giao diện (giao diện vẫn trống hoặc giữ IP cũ): Nó không thể tạo ra một gói tin ARP hợp lệ thuộc về subnet mới đó để gửi đi.
    - Do đó, nó bắt buộc phải chuyển sang trạng thái BOUND (tạm thời ghi nhận các thông số IP, Mask, Gateway, DNS vào bộ nhớ của card mạng) để hệ điều hành có đủ thông tin đóng gói (encapsulate) gói tin ARP.
    - Địa chỉ IP mà DHCP Server cấp trong gói DHCPACK được hiểu là "Server cam kết IP này hiện tại đang trống trong cơ sở dữ liệu của Server". Tuy nhiên, Server không thể biết được liệu trong mạng có ai đó tự ý đặt IP tĩnh (Static IP) trùng với số đó hay không.
    - Vì vậy, Client sẽ làm theo các bước: Áp đặt cấu hình: Gán yiaddr (Your IP Address) làm IP chính thức của giao diện mạng. Phát sóng kiểm tra (Gratuitous ARP): Client gửi một gói ARP chứa thông tin: "Tôi là IP X, địa chỉ MAC của tôi là Y". 
      - **Kịch bản 1 (Mạng sạch)**: Không ai trả lời -> Xác nhận IP an toàn -> Tiếp tục hoạt động bình thường.
      - **Kịch bản 2 (Bị trùng)**: Có một thiết bị khác trong mạng nhảy lên phản hồi: "Ơ kìa, tôi mới là IP X!" -> Client ngay lập tức hủy cấu hình vừa đặt, gửi gói DHCP DECLINE trả lại cho Server và quay về trạng thái INIT để xin IP khác.
    - Server không kiểm tra trước vì tốn tài nguyên Server: Một DHCP Server trong doanh nghiệp lớn hoặc nhà mạng phải cấp phát IP cho hàng nghìn Client cùng lúc. Nếu mỗi lần cấp, Server phải đứng đợi vài giây để Ping/ARP kiểm tra thì Server sẽ bị nghẽn (Timeout).
    - Góc nhìn của Server bị hạn chế: Server có thể ở xa Client (đi qua DHCP Relay Agent ở các tầng switch/router khác nhau). Server gửi ARP ở phân vùng mạng của nó thì không thể tìm ra thiết bị trùng IP đang nằm ở cùng phân vùng mạng (Local Subnet) với Client được. Chỉ có Client mới có góc nhìn chính xác nhất về môi trường xung quanh nó.
