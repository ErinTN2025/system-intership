...
- Server: Luôn luôn lắng nghe (Listen) cố định ở Cổng 21.
- Client: Sẽ mở một cổng ngẫu nhiên có số hiệu lớn (thường là > 1023, gọi là Ephemeral Port, ví dụ: Port X) để kết nối tới cổng 21 của Server.
...
**Active mode**:
- Ở chế độ này, Server là bên chủ động thiết lập kết nối dữ liệu ngược lại về phía Client.
- Client tự mở một cổng ngẫu nhiên thứ hai (ví dụ: Port X+1) để chuẩn bị nhận dữ liệu, sau đó gửi lệnh PORT qua kênh Control để báo cho Server biết: "Tôi đang đợi ở cổng X+1 nhé".
- Server sẽ chủ động mở Cổng 20 của mình và tạo một kết nối tới cổng X+1 của Client để truyền dữ liệu.
- Trong Active Mode: Client dùng port bất kỳ (X+1), Server dùng port cố định 20.
```arduino
...
**Passive Mode**:
- Do hầu hết Client ngày nay đều nằm sau Firewall hoặc NAT (Router ở nhà), Server không thể chủ động "bắn" kết nối từ ngoài vào trong máy Client được (bị Firewall chặn). Vì vậy, Passive Mode ra đời để Client chủ động kết nối ra ngoài hoàn toàn.
- Client gửi lệnh PASV qua kênh Control để bảo với Server: "Tôi bị chặn rồi, ông mở port trước đi để tôi kết nối vào".
- Server lúc này KHÔNG dùng cổng 20 nữa, mà nó sẽ mở một cổng ngẫu nhiên bất kỳ có số hiệu lớn (ví dụ: Port Y, thường nằm trong dải được cấu hình trước như 40000-50000) rồi báo số port này lại cho Client.
- Client mở một cổng ngẫu nhiên mới (X+2) và chủ động kết nối tới cổng Y của Server để truyền dữ liệu.
- Trong Passive Mode: Cả Client và Server đều sử dụng port ngẫu nhiên cho kênh dữ liệu. Cổng 20 hoàn toàn không được động tới.
