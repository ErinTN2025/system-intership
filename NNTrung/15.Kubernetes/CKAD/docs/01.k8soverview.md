...
#### 3.2.1 CRI-O
CRI-O (Container Runtime Interface - OCI) là một container runtime lightweight (nhẹ) được thiết kế dành riêng cho Kubernetes.

Nó nằm giữa `kubelet` và OCI runtime như `runc`/`crun`.

Nhiệm vụ duy nhất của CRI-O là làm cầu nối giúp Kubelet (agent của Kubernetes) giao tiếp trực tiếp với low-level container runtime (như runc) để pull image, tạo, chạy và quản lý các Pod/Container theo đúng chuẩn OCI (Open Container Initiative).

![altimage](../images/Screenshot_36.png)

kubelet không tự biết cách tạo Linux container. Nó nói chuyện với container runtime thông qua chuẩn CRI – Container Runtime Interface.

Trước đây, Kubernetes dùng Docker làm container runtime mặc định. Tuy nhiên, Docker là một bộ công cụ rất lớn, chứa nhiều tính năng không cần thiết cho Kubernetes (như CLI, Swarm, build tool...).

Khi Kubernetes giới thiệu CRI (Container Runtime Interface) — một chuẩn API chung cho phép Kubelet kết nối với bất kỳ container runtime nào — CRI-O đã ra đời với tiêu chí: Chỉ làm đúng những gì Kubernetes cần, không thừa không thiếu.

CRI-O nhận yêu cầu từ kubelet rồi lo các việc như quản lý container/pod sandbox, image và gọi OCI runtime (`runc`) để thực sự tạo container.

CRI-O = quản lý và điều phối việc chạy container.
runc/crun = thằng trực tiếp tạo container ở Linux level.

CRI-O cắt bỏ các lớp trung gian cồng kềnh và tối ưu luồng xử lý:

- Kubelet nhận chỉ thị từ Control Plane và gửi request đến CRI-O qua giao thức gRPC (dựa trên chuẩn CRI API).
- CRI-O tải (pull) container image về bằng thư viện containers/image.
- CRI-O chuẩn bị hệ thống lưu trữ/disk bằng containers/storage.
- CRI-O gọi low-level runtime (thường là runc hoặc crun) để khởi tạo và chạy các container bên trong Pod.
- Quản lý mạng cho Pod thông qua CNI (Container Network Interface).

| Tiêu chí| CRI-O| Containerd |
|----------|--------|----------|
| Mục đích| Thiết kế duy nhất cho Kubernetes.| "Tổng quát hơn (dùng được cho cả Kubernetes, Docker, Standalone hosts)."|
| Hệ sinh thái| "Dự án thuộc CNCF (Graduated), được hậu thuẫn lớn bởi Red Hat (mặc định trên Red Hat OpenShift)."| "Dự án thuộc CNCF (Graduated), phát triển bởi Docker/CNCF community."|
| Tốc độ cập nhật| Đồng bộ nhịp nhàng theo đúng phiên bản và release cycle của Kubernetes.| Phát triển theo chu kỳ độc lập.|
| Độ tối ưu| "Rất gọn nhẹ, tối đa hóa hiệu năng và bảo mật cho cluster."| "Đa năng, giàu tiện ích mở rộng hơn."|
| Có thể dùng | `runc, crun`,... | `runc`|
...
Ưu Điểm Chính Của CRI-O
- Tối ưu hiệu năng & tài nguyên: Do không phải gánh các service phụ trợ như Docker daemon, CRI-O tiêu tốn ít RAM/CPU hơn.
- Tuân thủ chuẩn OCI: Tương thích hoàn toàn với các image được build từ Docker, Podman hoặc bất kỳ công cụ build chuẩn OCI nào.
- Tích hợp chặt chẽ với Kubernetes: Phiên bản của CRI-O tuân theo chính xác versioning của Kubernetes (ví dụ: CRI-O v1.30 dành cho Kubernetes v1.30), giúp tránh các lỗi bất đồng phiên bản.
