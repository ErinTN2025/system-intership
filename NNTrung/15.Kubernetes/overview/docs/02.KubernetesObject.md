# Kubernetes Object
## 1. Khái niệm
Kubernetes Object là các thực thể tồn tại lâu dài (persistent entities) trong hệ thống Kubernetesdùng để đại diện cho trạng thái của cluster.
...
- **Name**: chuỗi do client đặt, dùng trong URL API (VD: `/api/v1/pods/my-pod`).
  ...
  - `generateName`: server tự sinh hậu tố ngẫu nhiên thay vì bạn đặt tên cố định (có thể vẫn đụng tên → lỗi 409, nhưng từ v1.31 thử tối đa 8 lần).s
...
kubectl config set-context --current --namespace=<ns>   # đặt namespace mặc định lâu dài
```
