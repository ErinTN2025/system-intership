# Multi Container
## 1. Lý do cần thiết kế Pod với nhiều Container
...
![altimgae](../images/init-container-2.gif)

### 2.1 Cấu hình Init container trong YAML
```yaml
apiVersion: v1
kind: Pod
metadata:
...
### 3.0 Cấu hình Sidecar trong YAML
```yaml
apiVersion: v1
kind: Pod
metadata:
...
## 4. Thiết kế Multi-Container Pod trong YAML
```YAML
apiVersion: v1
kind: Pod
metadata:
